...
export default function AdminDashboard() {
  ...
  const checkAuth = async () => {
    console.log("[v0] Admin: Checking authentication")
    try {
      const {
        data: { session },
        error,
      ...
      if (!session) {
        console.log("[v0] Admin: No session found")
        router.push("/auth/login")
        return
      }

      console.log("[v0] Admin: User authenticated:", session.user.id)
      setUser(session.user as User)

      // Load dashboard stats
      await loadStats()
    } catch (error) {
      console.log("[v0] Admin: Error checking auth:", error)
      router.push("/auth/login")
    } finally {
      setLoading(false)
    }
